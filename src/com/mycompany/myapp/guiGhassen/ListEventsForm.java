...
public class ListEventsForm extends Form {
    ...
    public ListEventsForm(Form previous, Resources res) {
        ...
        //fab.addActionListener(e -> new AddEventForm(this).show());
        FloatingActionButton sub = fab.createSubFAB(FontImage.MATERIAL_ARROW_BACK, "");
        sub.addActionListener(ex -> new EventMenu(res).show());
        FloatingActionButton sub2 = fab.createSubFAB(FontImage.MATERIAL_ARTICLE, "");
        sub2.addActionListener(e -> new AddEventForm(this, res).show());
        FloatingActionButton sub3 = fab.createSubFAB(FontImage.MATERIAL_REFRESH, "");
        sub3.addActionListener(et -> new ListEventsForm(previous, res).show());
    ...
    Display.getInstance () 
        ...
        for (Event eyy : en) {
            //System.out.println(eyy.getIdCoach());
            if(263==eyy.getIdCoach()){
            ...
            System.out.println(eyy.getDb_map());
            Image placeholder = Image.createImage(100, 100, 0xbfc9d2); //square image set to 10% of screen width
            EncodedImage encImage = EncodedImage.createFromImage(placeholder, false);
            Date date = new Date();
            long timeMilli = date.getTime();
            Image pic = URLImage.createToStorage(encImage, "pic" + String.valueOf(eyy.getId()), "http://" + eyy.getDb_map(), URLImage.RESIZE_SCALE);
            if (pic != null) {
                m.setIcon(pic);
            }
            m.addActionListener((evt) -> {

                updateEvent a = new updateEvent(previous, eyy.getNom_event(), Integer.toString(eyy.getNb_participant()), eyy.getDescription(), eyy.getParticipation(), eyy.getDate_debut(), eyy.getDate_fin(), eyy.getHeure_debut(), eyy.getHeure_fin(), Integer.toString(eyy.getId()), eyy.getDb_map(),eyy.getIdCoach(), res);
                System.out.println(eyy.getIdCoach());
                a.show();
